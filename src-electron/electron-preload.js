// src-electron/electron-preload.js
import { contextBridge, ipcRenderer } from 'electron';

contextBridge.exposeInMainWorld('db', {
  addDocument: (documentData) => ipcRenderer.invoke('add-document', documentData),
  addEvidence: (evidenceData) => ipcRenderer.invoke('add-evidence', evidenceData),
  addReference: (referenceData) => ipcRenderer.invoke('add-reference', referenceData),
  addStatement: (statementData) => ipcRenderer.invoke('add-statement', statementData),
  deleteDocument: (id) => ipcRenderer.invoke('delete-document', id),
  deleteEvidence: (id) => ipcRenderer.invoke('delete-evidence', id),
  deleteReference: (id) => ipcRenderer.invoke('delete-reference', id),
  deleteStatement: (id) => ipcRenderer.invoke('delete-statement', id),
  exportDocumentAsDocx: (id) => ipcRenderer.invoke('export-document-as-docx', id),
  exportDocumentAsMarkdown: (id) => ipcRenderer.invoke('export-document-as-markdown', id),
  exportFullBackup: () => ipcRenderer.invoke('export-full-backup'),
  exportSharedDocument: (id) => ipcRenderer.invoke('export-shared-document', id),
  getAllDocuments: () => ipcRenderer.invoke('get-all-documents'),
  getAllEvidence: () => ipcRenderer.invoke('get-all-evidence'),
  getAllReferences: () => ipcRenderer.invoke('get-all-references'),
  getAllStatements: () => ipcRenderer.invoke('get-all-statements'),
  getAllStatementsForLinking: () => ipcRenderer.invoke('get-all-statements-for-linking'),
  getAllTags: () => ipcRenderer.invoke('get-all-tags'),
  getDocumentDetails: (id) => ipcRenderer.invoke('get-document-details', id),
  getEvidenceDetails: (id) => ipcRenderer.invoke('get-evidence-details', id),
  getFilteredEvidence: (filters) => ipcRenderer.invoke('get-filtered-evidence', filters),
  getFilteredReferences: (filters) => ipcRenderer.invoke('get-filtered-references', filters),
  getFilteredStatements: (filters) => ipcRenderer.invoke('get-filtered-statements', filters),
  getReferenceDetails: (id) => ipcRenderer.invoke('get-reference-details', id),
  getStatementDetails: (id) => ipcRenderer.invoke('get-statement-details', id),
  getStatementLinks: (statementId) => ipcRenderer.invoke('get-statement-links', statementId),
  getTagsForEvidence: (evidenceId) => ipcRenderer.invoke('get-tags-for-evidence', evidenceId),
  importFromBibtex: () => ipcRenderer.invoke('import-from-bibtex'),
  importFullBackup: () => ipcRenderer.invoke('import-full-backup'),
  importSharedDocument: () => ipcRenderer.invoke('import-shared-document'),
  linkEvidenceToStatement: (linkData) => ipcRenderer.invoke('link-evidence-to-statement', linkData),
  linkStatementsToDocument: (linkData) => ipcRenderer.invoke('link-statements-to-document', linkData),
  mergeFullBackup: () => ipcRenderer.invoke('merge-full-backup'),
  universalSearch: (term) => ipcRenderer.invoke('universal-search', term),
  unlinkEvidenceFromStatement: (linkData) => ipcRenderer.invoke('unlink-evidence-from-statement', linkData),
  unlinkStatementLink: (data) => ipcRenderer.invoke('unlink-statement-link', data),
  updateDocument: (docData) => ipcRenderer.invoke('update-document', docData),
  updateEvidence: (evidenceData) => ipcRenderer.invoke('update-evidence', evidenceData),
  updateEvidenceTags: (data) => ipcRenderer.invoke('update-evidence-tags', data),
  updateReference: (referenceData) => ipcRenderer.invoke('update-reference', referenceData),
  updateStatement: (statementData) => ipcRenderer.invoke('update-statement', statementData),
  updateStatementLinks: (data) => ipcRenderer.invoke('update-statement-links', data),
});